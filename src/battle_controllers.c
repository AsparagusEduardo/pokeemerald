...
{
    bool32 isControllerPlayer = IsControllerPlayer(battler);
    SetSpritePrimaryCoordsFromSecondaryCoords(&gSprites[gBattleStruct->trainerSlideSpriteIds[battler]]);
    gSprites[gBattleStruct->trainerSlideSpriteIds[battler]].data[0] = isControllerPlayer ? 50 : 35;
...
void BtlController_Intro_TryShinyAnimShowHealthbox(u32 battler)
{
    ...
    else if (BattlerIsPartner(battler))
    {
        ...
        if (bgmRestored && battlerAnimsDone && ++gBattleSpritesDataPtr->healthBoxesData[battler].introEndDelay != 1)
        {
            gBattleSpritesDataPtr->healthBoxesData[battler].introEndDelay = 0;
            TryShinyAnimation(battler, GetBattlerMon(battler));
